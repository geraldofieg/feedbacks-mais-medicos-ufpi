...
* **`tarefas`:** `id`, `nomeTarefa` (Obrigatório), `enunciado` (Opcional), `urlEnunciado` (Opcional), `dataInicio`, `dataFim`, **`ano`** (Inteiro - indexador de ciclo escolar), `tipo` ('entrega', 'compromisso', 'lembrete'), `turmaId`, `instituicaoId`, `professorUid`, `status`, `dataCriacao`.
...
## 10. Gestão de Tarefas, Automação e Motor de Clonagem
...
* **O Motor de Clonagem (Turma Modelo):** Professores podem "Criar Turma a partir de Modelo". O sistema clona a "casca" estrutural de uma turma validada pelo Admin (`isModelo: true`), replicando 100% das tarefas (com nomes, prazos e enunciados originais) para o novo professor, mas **sem copiar os alunos**.
...
## 13. Gestão de Alunos e Matrículas Inteligentes
...
## 14. Cronograma Dinâmico e Fichas Técnicas SaaS
...
## 15. Padrões de Usabilidade e Navegação (Camadas de Defesa UX)
...
## 16. Acelerador de Fluxo: Botão Global (FAB)
