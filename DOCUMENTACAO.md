...
* **Persistência de Sessão (O Cérebro do Navegador):** * **Instituição:** O sistema salva a última instituição acessada no cache (`localStorage` sob a chave `@SaaS_EscolaSelecionada`).
  * **Turma Ativa (Sincronização Global):** O sistema salva a última turma manipulada no cache (`localStorage` via chave `ultimaTurmaAtiva`). Isso impede a "amnésia de rota", garantindo que as abas de Tarefas, Cronograma e Pendências abram sempre sincronizadas na mesma turma, sem resetar para a primeira da lista.
...
* **O Porteiro (Gatekeeper):** Se o professor não possuir uma instituição selecionada na sessão, o Dashboard exibe a interface focada de Instituições (Empty State de Nível 1).
* **Troca de Contexto Inteligente:** O Dashboard possui um *Dropdown* (Menu Suspenso) nativo no cabeçalho. Ele permite ao professor alternar instantaneamente entre suas Instituições.
* **Centro de Comando Mutante (Inversão de Hierarquia):** O layout adapta-se à maturidade do usuário:
    * Se `Turmas === 0`, o topo destaca convites de criação e oculta relatórios vazios.
    * Se `Turmas > 0`, os botões de criação descem ou reduzem de tamanho, e o topo passa a ser dominado por "Minhas Turmas Ativas" e pelo "Radar da Semana".
* **Radar da Semana (Mini-Cronograma):** Injetado na tela inicial, exibe até 3 eventos (Entregas, Reuniões ou Lembretes) programados para os próximos 7 dias, reduzindo a necessidade de cliques para checar a agenda.
...
* **Paginação Inteligente:** O sistema busca todos os alunos da turma na qual a tarefa foi lançada e cria botões de "Anterior" e "Próximo". Ao aprovar um feedback, o sistema avança automaticamente para o próximo aluno sem recarregar a tela.
* **Blindagem de Datas (Anti-Crash):** Utiliza um conversor de *timestamp* universal e à prova de falhas para evitar a "tela branca" ao renderizar datas antigas ou em formatos mistos no Firebase.
* **Ciclo de Vida (Funis Condicionais):**
    1. **Sem Resposta:** Aluno ainda não enviou nada (Ação indisponível).
    2. **Pendente:** `status === 'pendente'` (Aguardando primeira leitura do professor).
    3. **Falta Devolver (Aprovado):** `status === 'aprovado'` (Professor já corrigiu e deu nota, mas os dados estão ocultos para o aluno).
    4. **Finalizado (Devolvido):** `status === 'devolvido'` ou `postado: true` (Feedback liberado).
...
* *(Regras de Data mantidas integralmente: Vencido, Início, Meio e Reta Final)*
* **Templates Individuais:** Devem usar obrigatoriamente o primeiro nome do aluno.
* **Ações de Disparo (UX):** Copiar texto para o Moodle ou abrir Zap Privado (`wa.me`).

## 10. Mapa de Entregas e Pendências
* **Mapa:** Renderiza uma tabela dinâmica com alunos (linhas) e tarefas (colunas), exibindo Checks Verdes ou X Vermelhos. Em dispositivos móveis, utiliza rolagem horizontal.
* **Pendências (Visão por Tarefa):** Agrupa alunos devedores sob o título de cada tarefa (filtrando apenas itens do tipo `entrega`). 
    ...
    * Permite ação direta de cobrança via atalho para o módulo de comunicação, pré-selecionando o aluno.

## 11. Gestão de Tarefas e Agenda (Inversão de Hierarquia)
O professor cadastra itens na página `/tarefas` com categorias distintas: `Entrega (Desafio)`, `Compromisso (Aula)` ou `Lembrete (Post-it)`.
* **Foco no Dia a Dia:** A página possui hierarquia invertida. A barra de busca e os registros já existentes aparecem no topo para gestão rápida, enquanto o formulário de "Nova Tarefa" é fixado no rodapé (mobile) ou na coluna lateral (desktop).
...
1. **Estado Vazio Educativo (Empty States):** Se o usuário acessar uma tela filha sem ter a entidade pai, o sistema bloqueia as tabelas vazias e exibe um "Call to Action".
2. **Criação "Just-in-Time":** Ao cadastrar um Aluno, se a `Turma` desejada não existir, o professor pode criá-la dinamicamente por um botão `+ Nova Turma` dentro do modal.
3. **Redução do Caminho do Clique (Teletransporte):** Cartões referentes a tarefas do tipo "Entrega" no Dashboard ou no Cronograma funcionam como hiperlinks mágicos. Um único clique redireciona o professor diretamente para a respectiva Estação de Correção (`/revisar/id`). Cartões de Compromisso/Lembrete são estáticos (apenas expansivos para leitura).
4. **Navegação Profunda e Breadcrumbs:** Telas internas usam o `<Breadcrumb />` integrado.
5. **CRUD Dinâmico e Soft Delete:** A exclusão usa "Soft Delete" (`status: 'lixeira'`).
6. **Proteção de Rota Invisível:** Ocultação de elementos sem contexto.
7. **Espião de Rotas:** O sistema rastreia o estado (`location.state`) oriundo dos cliques dos cartões para forçar componentes dropdown a atualizarem a turma instantaneamente, superando os atrasos de renderização do React.

## 14. Acelerador de Fluxo: Botão Global (FAB)
* **Atalho Flutuante:** Um botão "+" posicionado no canto inferior direito para acesso rápido à criação de Turmas, Tarefas e Alunos.
...
* **O Radar (A Gaveta de Post-its):** Tarefas *sem* prazo definido (Lembretes soltos) são renderizadas fixas no topo da tela, servindo como uma visão contínua para o professor.
* **A Linha do Tempo:** Entregas e Compromissos *com* dataFim são ordenados verticalmente (do mais próximo ao mais distante), utilizando bolinhas e badges coloridos para indicar proximidade, mantendo o filtro de "Ocultar Passados".
* 
