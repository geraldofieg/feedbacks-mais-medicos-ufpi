...
- 🤖 **Termômetro da IA:** Um painel analítico que calcula, em tempo real, a percentagem de feedbacks gerados pela Inteligência Artificial que foram aprovados pela professora sem nenhuma alteração. Inclui selos visuais em cada atividade indicando se o texto foi "✨ 100% IA" ou "✏️ Editado".
...
- 🚨 **Relatório de Pendências e Gestão à Vista:** Cruza a lista de alunos com as atividades entregues e dedura automaticamente quem está em falta. Integrado ao Dashboard para fácil visualização das tarefas em atraso.
- 📅 **Cronograma Oficial (Timeline):** Exibição interativa dos módulos e semanas, tanto síncronos quanto assíncronos. Mostra dias restantes de prazos oficiais, indicando atividades atuais, futuras ou passadas.
- 💬 **Central de Comunicação (Integração WhatsApp):** Agrupamento inteligente de alunos com atividades pendentes baseado no cronograma atual, com geração de mensagens de cobrança padronizadas prontas para enviar pelo WhatsApp – individualmente ou em lote no grupo da turma.
...
- `Dashboard.jsx`: Painel inicial inteligente que se adapta ao perfil logado e exibe o Termômetro da IA, cronogramas atuais e o quadro de gestão à vista de alunos pendentes.
- `Cronograma.jsx`: Linha do tempo oficial dos módulos (Assíncrono e Síncrono), com prazos oficiais.
- `Comunicacao.jsx`: Central de integração com o WhatsApp para envio de cobranças automáticas de tarefas atrasadas/pendentes baseando-se no cronograma.
...
1. Instale o React com Vite: `npm create vite@latest feedbacks -- --template react`
2. Instale dependências: `npm install firebase react-router-dom lucide-react`
3. Configure o Tailwind CSS.
4. Crie a estrutura de pastas (`/services`, `/contexts`, `/pages`, `/data`).
5. Cole o conteúdo guardado de cada ficheiro `.jsx` e `.js`.
6. Configure as regras de CORS no Google Cloud Shell.
7. Faça o deploy (ex: Vercel).
