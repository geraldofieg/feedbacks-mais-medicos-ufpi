...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const unsubInst = onSnapshot(query(collection(db, 'saas_instituicoes'), orderBy('nome', 'asc')), (snap) => setInstituicoes(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubTurmas = onSnapshot(query(collection(db, 'saas_turmas'), orderBy('nome', 'asc')), (snap) => setTurmas(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    return () => { unsubInst(); unsubTurmas(); };
  }, [currentUser]);

  // A MÁGICA DO RÁDIO ESTÁ AQUI NESTAS DUAS FUNÇÕES:
  const handleInstChange = (e) => {
    ...
    window.dispatchEvent(new Event('workspaceChanged')); // Grita no rádio!
  };
        ...
        <div className="hidden md:flex items-center justify-between h-16">
          <div className="flex items-center gap-6">
            <Link to="/" className="font-black text-indigo-900 flex items-center gap-2 mr-4">
              <div className="bg-indigo-600 text-white w-8 h-8 rounded-lg flex items-center justify-center"><GraduationCap size={20} /></div>
            ...
            <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-lg border border-gray-200">
              <div className="flex items-center gap-1.5 px-2">
                ...
                <select className="bg-transparent text-sm font-bold text-gray-700 outline-none cursor-pointer max-w-[150px] truncate" value={instSelecionada} onChange={handleInstChange}>
              ...
              <div className="flex items-center gap-1.5 px-2">
                ...
                <select className="bg-transparent text-sm font-bold text-gray-700 outline-none cursor-pointer max-w-[150px] truncate disabled:opacity-50" value={turmaSelecionada} onChange={handleTurmaChange} disabled={!instSelecionada}>
          ...
          <div className="flex items-center gap-6">
            {navLinks.map(link => (
              <Link key={link.path} to={link.path} className={`flex items-center gap-1.5 text-sm font-bold transition-all ${location.pathname === link.path ? 'text-indigo-600 border-b-2 border-indigo-600 py-5' : 'text-gray-500 hover:text-indigo-500 py-5'}`}>
                {link.icon} {link.label}
              </Link>
            ))}
            <button onClick={handleLogout} className="flex items-center gap-1.5 text-sm font-bold text-red-500 hover:text-red-700 ml-4 py-5 border-l border-gray-200 pl-6"><LogOut size={18} /> Sair</button>
          </div>
        </div>
        ...
        <div className="md:hidden">
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
             <Link to="/" className="font-black text-indigo-900 flex items-center gap-2">
              <div className="bg-indigo-600 text-white w-7 h-7 rounded-lg flex items-center justify-center"><GraduationCap size={16} /></div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto py-3 w-full" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            ...
            {navLinks.map(link => (
              <Link key={link.path} to={link.path} className={`shrink-0 flex items-center gap-1.5 text-xs font-bold whitespace-nowrap px-3 py-2 rounded-full transition-colors ${location.pathname === link.path ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                {link.icon} {link.label}
              </Link>
            ))}
          </div>
