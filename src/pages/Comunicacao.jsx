...
export default function Comunicacao() {
  ...
  // ── VARIAÇÃO ALEATÓRIA DE ABERTURA E FECHAMENTO ─────────────────────────────
  // Sorteado uma vez a cada vez que a página é aberta/recarregada — mantém o
  // corpo da mensagem (nome da tarefa, urgência) sempre igual, mas a "roupagem"
  // muda para não repetir sempre o mesmo texto.

  const ABERTURAS_GERAL = [
    'Olá, pessoal! 👋',
    'Oi, turma! Tudo certo?',
    'Bom dia (ou boa tarde/noite), pessoal!',
    'Fala, galera!',
    'Oi, pessoal, tudo bem?',
    'Passando por aqui rapidinho, pessoal.',
    'Oi, turma! Espero que estejam bem.',
    'E aí, pessoal!',
    'Olá a todos!',
    'Oi, pessoal! Só um lembrete rápido.',
  ];

  const ABERTURAS_INDIVIDUAL = [
    (nome) => `Olá, ${nome}! 👋`,
    (nome) => `Oi, ${nome}, tudo bem?`,
    (nome) => `Bom dia (ou boa tarde/noite), ${nome}!`,
    (nome) => `Fala, ${nome}!`,
    (nome) => `Oi, ${nome}! Espero que esteja tudo certo por aí.`,
    (nome) => `Passando por aqui rapidinho, ${nome}.`,
    (nome) => `${nome}, tudo bem?`,
    (nome) => `E aí, ${nome}!`,
    (nome) => `Olá, ${nome}!`,
    (nome) => `Oi, ${nome}! Só um lembrete rápido.`,
  ];

  const FECHAMENTOS = [
    'Qualquer dúvida, estou à disposição!',
    'Fico no aguardo. Qualquer coisa, me chame!',
    'Conto com vocês. Um abraço!',
    'Qualquer coisa, pode contar comigo.',
    'Fico à disposição para o que precisar.',
    'Vamos juntos! Qualquer dúvida, é só chamar.',
    'Obrigado(a) pela atenção. Qualquer coisa, me avise.',
    'Seguimos à disposição por aqui.',
    'Um abraço e bons estudos!',
    'Estou por aqui caso precisem de algo.',
  ];

  const [seedAbertura] = useState(() => Math.floor(Math.random() * ABERTURAS_GERAL.length));
  const [seedFechamento] = useState(() => Math.floor(Math.random() * FECHAMENTOS.length));

  const aberturaGeral = ABERTURAS_GERAL[seedAbertura];
  const aberturaIndividual = (nome) => ABERTURAS_INDIVIDUAL[seedAbertura](nome);
  const fechamento = FECHAMENTOS[seedFechamento];
  ...
  const gerarMensagemGeral = () => {
    ...
    if (tarefaAtivaId === 'todas') {
      ...
      if (minDiasGeral !== null) {
          if (minDiasGeral < 0) corpo = `Notei no sistema que temos atividades com o prazo já encerrado e outras pendências acumuladas na turma.\n\nPor favor, deem uma olhada no portal e regularizem imediatamente para evitarmos problemas com a aprovação.`;
          else if (minDiasGeral >= 20) corpo = `Passando para lembrar das nossas atividades em andamento. Temos algumas pendências na turma.\n\nAinda temos um bom prazo (a entrega mais próxima encerra em ${minDiasGeral} dias), mas recomendamos ir adiantando as atividades.`;
          else if (minDiasGeral >= 8) corpo = `Nosso lembrete de acompanhamento. Notei algumas pendências no sistema.\n\nA entrega mais próxima encerra em ${minDiasGeral} dias. Vamos aproveitar os próximos dias para colocar tudo em dia!`;
          else corpo = `🚨 Passando para alertar que entramos na reta final de algumas atividades!\nA entrega mais próxima vence em apenas ${minDiasGeral} dias e ainda temos pendências.\n\nPeço a regularização o quanto antes para evitarmos problemas.`;
      } else {
          corpo = `Notei no sistema que temos algumas pendências acumuladas nas entregas.\n\nPor favor, deem uma olhada no portal e regularizem as atividades o quanto antes para evitarmos problemas com a aprovação.`;
      }
      return `${aberturaGeral}\n${corpo}\n${fechamento}`;
    }

    if (!tarefaAtualObj) return '';
    let corpo;
    if (diasRestantesVisual !== null) {
      if (diasRestantesVisual < 0) corpo = `O prazo oficial de *${nomeTarefa}* foi encerrado.\nNotei algumas pendências no sistema.\n\nPor favor, regularizem as entregas imediatamente para evitarmos problemas com a aprovação.`;
      else if (diasRestantesVisual >= 20) corpo = `Passando para avisar que a etapa de *${nomeTarefa}* já está em andamento.\n\nFaltam ${diasRestantesVisual} dias para o encerramento.\nQuem já quiser ir adiantando as atividades, desejo excelentes estudos!`;
      else if (diasRestantesVisual >= 8) corpo = `Nosso lembrete de acompanhamento sobre *${nomeTarefa}*.\n\nEntramos na fase intermediária e faltam ${diasRestantesVisual} dias para o encerramento.\nVamos aproveitar os próximos dias para colocar tudo em dia!`;
      else corpo = `🚨 Passando para alertar que entramos na reta final de *${nomeTarefa}*.\nFaltam apenas ${diasRestantesVisual} dias para o encerramento!\n\nPeço a regularização das tarefas pendentes o quanto antes para evitarmos problemas.`;
    } else {
      corpo = `Passando para lembrar do nosso acompanhamento sobre *${nomeTarefa}*.\nPeço a regularização das tarefas pendentes o quanto antes para não acumular.`;
    }
    return `${aberturaGeral}\n${corpo}\n${fechamento}`;
  };
  ...
  const gerarMensagemIndividual = (aluno) => {
    ...
    const abertura = aberturaIndividual(primeiroNome);
    ...
    if (tarefaAtivaId === 'todas') {
        ...
        if (dias !== null && dias !== undefined) {
            if (dias < 0) corpo = `Notei no sistema que você possui atividades com o prazo já encerrado e outras pendências:\n\n${listaFormatada}\n\nPor favor, regularize essas entregas imediatamente para evitarmos problemas com a aprovação.`;
            else if (dias >= 20) corpo = `Passando para avisar que você tem as seguintes atividades em andamento:\n\n${listaFormatada}\n\nAinda temos um bom prazo (a mais próxima encerra em ${dias} dias), mas recomendo adiantar a execução para não acumular.`;
            else if (dias >= 8) corpo = `Nosso lembrete de acompanhamento. Você possui as seguintes pendências:\n\n${listaFormatada}\n\nA atividade mais próxima encerra em ${dias} dias. Vamos aproveitar os próximos dias para colocar tudo em dia!`;
            else corpo = `🚨 Passando para alertar que entramos na reta final para algumas de suas pendências:\n\n${listaFormatada}\n\nA entrega mais próxima vence em apenas ${dias} dias! Recomendo que regularize o quanto antes para não acumular nem termos problemas.`;
        } else {
            corpo = `Notei no sistema que você possui pendências nas seguintes atividades:\n\n${listaFormatada}\n\nPor favor, regularize essa situação o quanto antes para não acumular e evitarmos problemas com a aprovação.`;
        }
        return `${abertura}\n${corpo}\n${fechamento}`;
    }
    ...
    let corpo;
    if (diasRestantesVisual !== null) {
      if (diasRestantesVisual < 0) corpo = `O prazo oficial de *${nomeTarefa}* foi encerrado. Notei no sistema que ainda consta pendência para a entrega desta atividade.\n\nPor favor, regularize essa situação imediatamente para evitarmos problemas com a aprovação.`;
      ...
      else corpo = `🚨 Passando para alertar que entramos na reta final de *${nomeTarefa}*.\nFaltam apenas ${diasRestantesVisual} dias para o encerramento!\n\nNotei pendência para esta entrega.\nRecomendo que regularize o quanto antes para não acumular nem termos problemas.`;
    } else {
      corpo = `Passando para lembrar do nosso acompanhamento sobre *${nomeTarefa}*.\nNotei pendência para esta entrega.\n\nRecomendo a regularização o quanto antes para não acumular.`;
    }
    return `${abertura}\n${corpo}\n${fechamento}`;
  };
