...
export default function Tarefas() {
  ...
  // Conversor Universal de Data (Lida com Timestamps, Datas JS e Strings)
  const tsToInput = (ts) => {
    if (!ts) return "";
    try {
      let d;
  ...
  const prepararEdicao = (tarefa) => {
    const nome = tarefa.nomeTarefa || tarefa.titulo || 'Sem nome';
    const tiposValidos = ['entrega', 'compromisso', 'lembrete'];
    const tipoAtual = (tarefa.tipo || 'entrega').toLowerCase();

    setEditandoId(tarefa.id);
    setTituloEdicao(nome);
    setEnunciadoEdicao(tarefa.enunciado || '');
    setDataFimEdicao(tsToInput(tarefa.dataFim));
    // Se o tipo for antigo (ex: 'prática'), converte para 'entrega' para não travar o menu
    setTipoEdicao(tiposValidos.includes(tipoAtual) ? tipoAtual : 'entrega');
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-24">
            ...
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Novo Registro</h2>
                ...
                {editandoId === tarefa.id ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <input className="w-full border-2 border-blue-500 rounded-xl px-3 py-2 font-bold" value={tituloEdicao} onChange={e => setTituloEdicao(e.target.value)}/>
