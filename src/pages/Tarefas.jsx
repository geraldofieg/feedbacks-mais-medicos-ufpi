...
export default function Tarefas() {
  ...
  const tsToInput = (ts) => {
    ...
    switch(tipo) {
      ...
      default: return <FileText size={22} className="text-gray-400" />;
    }
  };
  ...
  const getCorTipo = (tipo) => {
    switch(tipo) {
      ...
      default: return 'border-gray-200 bg-gray-50/10';
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-24">
            ...
            <form onSubmit={handleCriar} className="space-y-4">
              ...
              <input type="text" required placeholder="Título..." className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={novaTarefa.titulo} onChange={e => setNovaTarefa({...novaTarefa, titulo: e.target.value})} />
              <input type="datetime-local" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={novaTarefa.dataFim} onChange={e => setNovaTarefa({...novaTarefa, dataFim: e.target.value})} />
              <textarea placeholder="Observações..." rows="3" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium resize-none" value={novaTarefa.enunciado} onChange={e => setNovaTarefa({...novaTarefa, enunciado: e.target.value})} />
              <button disabled={salvando || !turmaAtiva} className="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition-all shadow-md flex items-center justify-center gap-2 disabled:opacity-50"> <Plus size={20}/> {salvando ? 'Salvando...' : 'Adicionar'} </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {loading ? ( <div className="p-10 text-center animate-pulse font-bold text-gray-400">Sincronizando...</div> ) : tarefasFiltradas.length === 0 ? ( <div className="text-center py-16 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200"> <p className="text-gray-400 font-medium">Nenhum registro para esta turma.</p> </div> ) : (
            <div className="grid grid-cols-1 gap-4">
              {tarefasFiltradas.map(tarefa => {
                const nomeSeguro = tarefa.nomeTarefa || tarefa.titulo || 'Sem nome';
                const tipoOriginal = tarefa.tipo || 'entrega';
                
                return (
                <div key={tarefa.id} className={`bg-white p-5 rounded-2xl border transition-all group ${editandoId === tarefa.id ? 'border-blue-400 shadow-md ring-2 ring-blue-50' : `${getCorTipo(tipoOriginal)} shadow-sm`}`}>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-3">
                          <div className="p-2.5 bg-white rounded-xl shadow-inner border border-gray-100">{getIconeTipo(tipoOriginal)}</div>
                          <div>
                            <h3 className="font-black text-gray-800 text-base leading-tight pr-4">{nomeSeguro}</h3>
                            <span className="text-[9px] font-black uppercase text-gray-400 tracking-widest">{tipoOriginal}</span>
                          </div>
                        </div>
                        <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                          <button 
                            onClick={() => { 
                              ...
                              setTituloEdicao(nomeSeguro); 
                              setEnunciadoEdicao(tarefa.enunciado || ''); 
                              setDataFimEdicao(tsToInput(tarefa.dataFim)); 
                              // TRAVA DE SEGURANÇA: Se o tipo for desconhecido (ex: 'prática'), força 'entrega'
                              const tiposValidos = ['entrega', 'compromisso', 'lembrete'];
                              setTipoEdicao(tiposValidos.includes(tipoOriginal) ? tipoOriginal : 'entrega');
                            }} 
                      ...
                      <div className="flex items-center justify-between gap-4 mt-2">
                        <p className="text-gray-500 text-xs line-clamp-2 font-medium italic flex-1">{tarefa.enunciado || "Sem observações."}</p>
                        <div className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl border font-bold text-[10px] ${tarefa.dataFim ? "bg-white text-gray-700 border-gray-100 shadow-sm" : "bg-gray-100 text-gray-400 border-transparent"}`}> <CalendarClock size={14} className={tarefa.dataFim ? "text-blue-500" : "text-gray-300"} /> {tarefa.dataFim ? formatarDataLocal(tarefa.dataFim) : "Sem prazo"} </div>
                      </div>
                    </div>
                  )}
                </div>
