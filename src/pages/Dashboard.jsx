...
export default function Dashboard() {
  ...
  // 🔥 CORREÇÃO DO ERRO DE F5: Variável restaurada
  const finalizadosVisor = isAdmin ? kanban.finalizados : (kanban.finalizados + kanban.faltaLancar);

  // 1. A BÚSSOLA DE LOGIN (RESTORE: Agora com detecção inteligente de turma)
  useEffect(() => {
    ...
    async function setupRadarGlobal() {
      ...
      try {
        ...
        // BUSCA AS TURMAS DO USUÁRIO PARA SABER ONDE ELE REALMENTE ENSINA
        ...
        if (turmasGlobais.length > 0) {
            ...
            if (temTurmaNoCache) {
                ...
                const idCerta = turmasGlobais[0].instituicaoId; // Pega a instituição da turma mais recente do professor
        ...
        console.error("Erro na bússola:", e); 
      } finally { 
        setLoadingInst(false); 
      }
    }
