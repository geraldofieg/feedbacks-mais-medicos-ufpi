...
export default function Pendencias() {
  ...
  useEffect(() => {
    async function fetchPendencias() {
      if (!turmaAtiva) { setPendencias([]); return; }
      ...
      try {
        ...
        if (alunosData.length === 0) { setPendencias([]); setLoadingDados(false); return; }
        ...
        const tarefasOrdenadas = tarefasData.sort((a, b) => {
          const sA = a.statusTemp; const sB = b.statusTemp;
          if (!sA.temPrazo && sB.temPrazo) return 1;
          if (sA.temPrazo && !sB.temPrazo) return -1;
          const rank = { vencida: 1, atual: 2, futura: 3 };
          if (rank[sA.categoria] !== rank[sB.categoria]) return rank[sA.categoria] - rank[sB.categoria];
          if (sA.categoria === 'vencida') return sB.timestampFim - sA.timestampFim;
          return sA.timestampFim - sB.timestampFim;
        });
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-blue-50 border-2 border-dashed border-blue-200 p-12 rounded-3xl text-center max-w-2xl mx-auto mt-10 shadow-sm">
          ...
          <Link to="/" className="inline-flex items-center justify-center gap-2 bg-blue-600 text-white font-black py-4 px-10 rounded-xl hover:bg-blue-700 shadow-lg">Ir para o Início</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 md:py-8">
      ...
      {isCarregando ? (
        <div className="p-20 text-center animate-pulse flex flex-col items-center gap-3">
          ...
          <p className="font-bold text-red-400 text-lg">Sincronizando entregas...</p>
        </div>
      ) : pendencias.length === 0 ? (
        ...
        <div className="space-y-6 mt-8">
          {pendencias.map((item, idx) => {
            ...
            let theme = status.categoria === 'vencida' ? 'red' : status.categoria === 'atual' ? 'orange' : 'gray';

            return (
              <div key={idx} className={`bg-white rounded-2xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow ${theme === 'red' ? 'border-red-200' : theme === 'orange' ? 'border-orange-200' : 'border-gray-200'}`}>
                <div className={`${theme === 'red' ? 'bg-red-50/80' : theme === 'orange' ? 'bg-orange-50/80' : 'bg-gray-50'} p-4 border-b flex flex-col md:flex-row md:items-center justify-between gap-3`}>
                  <div className="flex items-center gap-3">
                    <Calendar className={`${theme === 'red' ? 'text-red-500' : theme === 'orange' ? 'text-orange-500' : 'text-gray-500'} shrink-0`} size={24} />
                    <div>
                      <h3 className={`font-black text-lg ${theme === 'red' ? 'text-red-900' : theme === 'orange' ? 'text-orange-900' : 'text-gray-800'}`}>
                ...
                <div className="p-5 bg-white">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {item.devedores.map(aluno => (
                      <div key={aluno.id} onClick={() => navigate('/revisar/' + item.tarefa.id, { state: { alunoId: aluno.id } })} className="flex items-center justify-between gap-2 text-sm font-bold text-gray-700 bg-gray-50 hover:bg-blue-50 hover:border-blue-200 p-3 rounded-xl border border-gray-100 transition-colors group cursor-pointer shadow-sm">
                        <div className="flex items-center gap-2 truncate"> 
                          <User size={16} className="text-gray-400 group-hover:text-blue-500 shrink-0"/> 
                          <span className="truncate">{aluno.nome}</span> 
                        </div>
                        <div className="bg-white p-1.5 rounded-md border border-blue-200 text-blue-600 shadow-sm"><MessageCircle size={14}/></div>
                      </div>
                    ))}
                  </div>
                </div>
