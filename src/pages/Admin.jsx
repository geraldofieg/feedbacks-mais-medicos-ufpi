...
export default function Admin() {
  ...
  async function handleMudarCargo(id, cargoAtual) {
    const novoCargo = cargoAtual === 'admin' ?
      'professor' : 'admin';
...
border border-slate-200 text-slate-700 text-sm rounded-xl py-2.5 pl-10 pr-8 font-bold outline-none focus:border-blue-500 appearance-none shadow-sm cursor-pointer"
            >
...
md:table">
            <thead className="hidden md:table-header-group">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="p-6 text-sm font-black text-slate-800 
uppercase tracking-widest">Plano de Assinatura</th>
                ...
                return (
                  <tr key={user.id} className={`flex flex-col md:table-row hover:bg-slate-50/50 transition-colors group p-4 md:p-0 ${user.status === 'bloqueado' ? 'opacity-70' : ''}`}>
                    ...
                    <td className="p-2 md:p-6 flex flex-col md:table-cell border-b border-slate-100 md:border-none mb-4 md:mb-0 pb-4 md:pb-0 last:border-0 last:mb-0 last:pb-0">
                      ...
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex 
items-center justify-center shrink-0 shadow-sm ${user.role === 'admin' ? 'bg-yellow-100 text-yellow-600' : 'bg-blue-100 text-blue-600'}`}>
                          {user.role === 'admin' ? <Crown size={22}/> : <User size={22}/>}
                        </div>
                        <div className="min-w-0 flex-1">
    
                          <div className="flex items-center gap-2">
                            <p className="font-black text-slate-900 text-lg leading-tight truncate">{user.nome ||
                              'Sem Nome'}</p>
                    ...
                    <td className="p-2 md:p-6 flex flex-col md:table-cell border-b border-slate-100 md:border-none mb-4 md:mb-0 pb-4 md:pb-0 last:border-0 last:mb-0 last:pb-0">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {user.role === 'admin' ?
                            <><Crown size={14}/> Super Admin</> : <><UserCog size={14}/> Professor</>}
                        </button>
                      </div>
                    </td>

                    <td className="p-2 md:p-6 flex flex-col md:table-cell border-b 
border-slate-100 md:border-none mb-4 md:mb-0 pb-4 md:pb-0 last:border-0 last:mb-0 last:pb-0">
                      ...
                      <div className="relative inline-block w-full md:w-auto">
                        <Zap className={`absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none ${user.plano === 'premium' ?
                          'text-yellow-500' : 'text-slate-300'}`} size={16} />
                        <select 
                          value={user.plano ||
                            'basico'} 
                          disabled={user.status === 'bloqueado'}
                          onChange={(e) => handleMudarPlano(user.id, e.target.value)}
                          className={`w-full md:w-auto bg-slate-50 border-2 border-slate-100 text-slate-700 text-sm rounded-xl py-2.5 pl-10 pr-4 font-black outline-none focus:border-blue-500 appearance-none shadow-sm 
min-w-[200px] ${user.status === 'bloqueado' ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
                        >
...
${textoDiasRestantes.includes('Vencido') ? 'text-red-500' : 'text-orange-500'}`}>
                              {textoDiasRestantes}
                            </p>
                          )}
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-1">
                          ...
                          <button 
                            ...
                            title={user.isVitalicio 
                              ? "Revogar Acesso Permanente" : "Tornar Vitalício"}
                          >
                            {user.isVitalicio ?
                              <><XCircle size={14} /> Revogar</> : <><Infinity size={14} /> Vitalício</>}
                          </button>
                        </div>
                      </div>
                    ...
                    <td className="p-2 md:p-6 flex flex-col md:table-cell last:border-0 last:mb-0 last:pb-0">
                      ...
                      <div className="flex flex-col md:items-end gap-2 w-full">
                        ...
                        <button 
                          ...
                          className={`flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl 
transition-all shadow-sm w-full md:w-auto justify-center border ${
                          ...
                          title={user.status === 'bloqueado' ?
                            "Liberar Acesso do Usuário" : "Bloquear Acesso do Usuário"}
                        >
                          {user.status === 'bloqueado' ?
                            <><UserCheck size={14}/> Reativar</> : <><Ban size={14}/> Suspender</>}
                        </button>
