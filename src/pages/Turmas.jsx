...
export default function Turmas() {
  ...
  // 🔥 NOVOS ESTADOS: GESTÃO DE INSTITUIÇÃO
  ...
  async function handleSalvarEdicaoEscola() {
    ...
    } catch (error) { console.error("Erro ao editar instituição:", error); }
    finally { setSalvandoEscola(false); }
  }

  async function handleLixeiraEscola() {
    if (!window.confirm(`⚠️ ATENÇÃO: Deseja enviar a instituição "${escolaSelecionada.nome}" para a lixeira?\n\nIsso ocultará todas as turmas e alunos vinculados a ela.`)) return;
    try {
      await updateDoc(doc(db, 'instituicoes', escolaSelecionada.id), { status: 'lixeira' });
      setEscolaSelecionada(null);
      window.location.href = '/'; // Reset completo
  ...
  function cancelarCriacaoEscola() {
     setPrecisaCriarEscola(false);
     setNovaEscolaNome('');
     if (!escolaSelecionada) navigate('/');
  }

  // =========================================================================
  // GESTÃO DE TURMAS (NÍVEL 2)
  // =========================================================================
  ...
  async function handleClonarTurma(e) {
    ...
    try {
      ...
      setModeloSelecionado(''); setNomeTurmaClonada(''); setModoCriacao('nova');
      alert(`✅ Sucesso! Turma criada e ${tarefasCopiadas} tarefas importadas.`);
    } catch (error) { console.error("Erro ao clonar:", error); alert("Erro ao importar modelo."); } 
  ...
  async function handleLixeira(id, nome) {
    if (!window.confirm(`Deseja enviar a turma "${nome}" para a lixeira?`)) return;
    try {
  ...
  if (precisaCriarEscola) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center animate-in zoom-in-95 duration-500">
        ...
        <p className="text-gray-500 text-lg mb-10 max-w-lg mx-auto font-medium">Ex: UFPI, UFT, ou "Consultoria Particular".</p>
        <form onSubmit={handleCriarInstituicao} className="max-w-md mx-auto flex flex-col gap-4">
          <input
            ...
            className="w-full px-5 py-4 bg-white border-2 border-gray-200 text-gray-800 rounded-2xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 font-bold outline-none text-lg transition-all text-center"
          ...
          <button type="submit" disabled={salvandoEscola} className="w-full bg-blue-600 text-white font-black py-4 px-8 rounded-2xl shadow-lg hover:bg-blue-700 disabled:opacity-50 transition-all text-lg flex justify-center items-center gap-2">
            {salvandoEscola ? 'Criando...' : 'Salvar e Continuar'} <ArrowRight size={20}/>
          </button>
          {escolaSelecionada && (
            <button type="button" onClick={cancelarCriacaoEscola} className="mt-2 text-sm font-bold text-gray-400 hover:text-gray-600 transition-colors">Cancelar e voltar</button>
          )}
        </form>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 mt-3 border-b border-gray-200 pb-6">
        <div className="flex items-center gap-3">
            <div className="bg-blue-100 text-blue-700 p-3 rounded-xl shadow-sm"><BookOpen size={28} /></div>
            <div>
              <h1 className="text-2xl font-black text-gray-800 leading-tight">Gestão de Turmas</h1>
              
              {/* 🔥 EDIÇÃO DE INSTITUIÇÃO INLINE */}
              {editandoEscola ? (
                <div className="flex items-center gap-2 mt-1">
                   <input 
                      type="text" 
                      className="border border-blue-300 rounded-lg px-2 py-1 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500" 
                      value={nomeEscolaEdicao} 
                      onChange={(e) => setNomeEscolaEdicao(e.target.value)}
                      autoFocus
                   />
                ...
                <div className="flex items-center gap-2 mt-0.5 group">
                  ...
                  <button onClick={() => { setEditandoEscola(true); setNomeEscolaEdicao(escolaSelecionada.nome); }} className="p-1 text-gray-300 hover:text-blue-500 transition-colors"><Pencil size={14}/></button>
                  <button onClick={handleLixeiraEscola} className="p-1 text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                </div>
              )}
            </div>
        </div>
        
        <button onClick={() => setPrecisaCriarEscola(true)} className="flex items-center justify-center gap-2 bg-white border border-gray-200 text-gray-600 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all text-sm"><Building2 size={16} /> Nova Instituição</button>
      </div>
              ...
              {turmas.map(turma => (
                <div key={turma.id} className={`bg-white border rounded-2xl shadow-sm transition-all overflow-hidden flex flex-col group ${turma.isModelo ? 'border-yellow-300 shadow-yellow-500/10' : 'border-gray-200 hover:border-blue-300'}`}>
                  <div className="p-5 flex-1 relative">
                    {isAdmin && (
                      <button onClick={() => toggleTurmaModelo(turma.id, turma.isModelo)} className={`absolute top-4 right-4 p-2 rounded-full transition-all shadow-sm ${turma.isModelo ? 'bg-yellow-100 text-yellow-500 hover:bg-yellow-200' : 'bg-gray-50 text-gray-300 hover:bg-gray-100 hover:text-gray-500'}`}><Star size={18} fill={turma.isModelo ? "currentColor" : "none"} /></button>
                    )}
                    <div className="flex justify-between items-start mb-3 pr-10">
                      ...
                      <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditandoId(turma.id); setNomeEdicao(turma.nome); }} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Pencil size={18}/></button>
                        <button onClick={() => handleLixeira(turma.id, turma.nome)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={18}/></button>
                      </div>
                    </div>
                    {editandoId === turma.id ? (
                      <div className="flex items-center gap-2 mb-1 mt-2">
                        <input type="text" value={nomeEdicao} onChange={(e) => setNomeEdicao(e.target.value)} className="w-full border border-blue-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-bold text-gray-800" autoFocus />
                  ...
                  <div className="bg-gray-50 border-t border-gray-100 p-3 grid grid-cols-2 gap-2">
                    <Link to="/alunos" state={{ turmaIdSelecionada: turma.id }} className="flex items-center justify-center gap-1.5 text-sm font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 py-2 rounded-lg transition-colors"><Users size={16}/> Alunos</Link>
                    <Link to="/tarefas" state={{ turmaIdSelecionada: turma.id }} className="flex items-center justify-center gap-1.5 text-sm font-bold text-blue-600 hover:bg-blue-50 py-2 rounded-lg transition-colors"><FileText size={16}/> Tarefas <ArrowRight size={14}/></Link>
                  </div>
                </div>
              ))}
           </div>
        ...
        <div className="w-full lg:w-1/3 order-2">
          <div className="bg-white p-2 rounded-3xl border border-gray-200 shadow-sm sticky top-24">
            <div className="flex bg-gray-50 p-1 rounded-2xl mb-4">
              <button onClick={() => setModoCriacao('nova')} className={`flex-1 py-2.5 rounded-xl text-sm font-black transition-all flex items-center justify-center gap-2 ${modoCriacao === 'nova' ? 'bg-white text-blue-600 shadow-sm border border-gray-100' : 'text-gray-400 hover:text-gray-600'}`}><Plus size={16}/> Criar do Zero</button>
              <button onClick={() => setModoCriacao('clonar')} className={`flex-1 py-2.5 rounded-xl text-sm font-black transition-all flex items-center justify-center gap-2 ${modoCriacao === 'clonar' ? 'bg-white text-purple-600 shadow-sm border border-gray-100' : 'text-gray-400 hover:text-gray-600'}`}><Copy size={16}/> Importar Modelo</button>
            </div>
            {modoCriacao === 'nova' && (
              <div className="p-4 animate-in fade-in duration-300">
                <form onSubmit={handleCriarTurma} className="flex flex-col gap-4">
                  <div>
                    ...
                    <input type="text" required placeholder="Ex: Odontologia 3º Período..." className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 text-gray-800 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 font-bold outline-none placeholder:font-medium placeholder:text-gray-400 transition-all" value={novaTurma} onChange={(e) => setNovaTurma(e.target.value)} />
                  </div>
                  <button type="submit" disabled={salvando} className="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-all shadow-sm flex justify-center items-center gap-2">{salvando ? <RefreshCw className="animate-spin" size={20}/> : <Plus size={20}/>} {salvando ? 'Criando...' : 'Criar Turma em Branco'}</button>
                </form>
              </div>
            )}
            {modoCriacao === 'clonar' && (
              <div className="p-4 animate-in fade-in duration-300">
                {turmasModelo.length === 0 ? (
                  <div className="text-center py-6 px-2 bg-purple-50 rounded-xl border border-purple-100"><Star className="mx-auto text-purple-300 mb-2" size={32}/><p className="text-sm font-bold text-purple-800 leading-tight">A coordenação ainda não definiu turmas-modelo para {escolaSelecionada?.nome}.</p></div>
                ) : (
                  <form onSubmit={handleClonarTurma} className="flex flex-col gap-4">
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Selecione o Modelo</label>
                      <select required className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 text-gray-800 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500 font-bold outline-none cursor-pointer" value={modeloSelecionado} onChange={e => setModeloSelecionado(e.target.value)}><option value="">Escolha um curso...</option>{turmasModelo.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}</select>
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Nome da sua Turma</label>
                      <input type="text" required placeholder="Ex: Mais Médicos - Polo Teresina" className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 text-gray-800 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500 font-bold outline-none placeholder:font-medium placeholder:text-gray-400 transition-all" value={nomeTurmaClonada} onChange={(e) => setNomeTurmaClonada(e.target.value)} />
                    </div>
                    <button type="submit" disabled={clonando || !modeloSelecionado} className="w-full bg-purple-600 text-white font-black py-4 rounded-xl hover:bg-purple-700 disabled:opacity-50 transition-all shadow-sm flex justify-center items-center gap-2 mt-2">{clonando ? <RefreshCw className="animate-spin" size={20}/> : <Copy size={20}/>} {clonando ? 'Importando tarefas...' : 'Clonar e Iniciar Curso'}</button>
                  </form>
                )}
              </div>
            )}
